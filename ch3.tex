...
The decorator pattern is thus used to implement the add VFS prototypes, such as encryption and versioning.
...
But since I currently don't require holding reference to underlying VFS I am able to have even simplified implementation without boiler-plate code.
As I use the CustomVfs it is more like a value type, meaning it's behaviour is defined not by its reference but the values inside it.
This allows me to simply copy construct a previous decorator that acts the same.
This might need to be changed in the future, and that is also the primary reason for mentioning this pattern.
...
This order decision has to be done by the person creating the result vfs but it's my responsibility to provide sufficient tools.
