...
Still, several virtual file systems (VFS) with versioning capabilities have been developed:
...
Third option for versioning is to use higher-level applications.
...
On truthfully, using these applications would be a practical solution, as they are well-maintained and offer a wide range of features.
...
Since we now established that the main competition is higher-level applications, let us examine why devevloing proposed VFS is still a reasonable idea even though one might label it as ``overkill''.
...
And even more importantly, users are limited to use that particular application, while with VFS they can use any application they want.
Yet, for the usage of VFS to be convinient, user's application of choice would need to be slightly modified.

On that note, is is also worth noting that some operating systems, specifically macOS, provide built-in support for versioning and limited encryption features.
