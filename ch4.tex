...
Debugging the tests was difficult in particular, since the \texttt{gdb} did not handle properly detached processes and would not stop on VFS breakpoints.
...
Because of that, I had to run the tests on a custom runner with privileged access, but as I do not want them to be running all the time, I limited the CI/CD pipeline to run only helper method tests.
